...
- **资源库**：卡片式展示已完成的分享链接，点击进入可浏览文件夹结构、搜索、转存
- **本地资源库**：文件元数据存储在本地 SQLite 数据库，不依赖网络即可搜索
- **高级筛选搜索**：按文件名、类型（视频/音频/图片/文档/ISO原盘/字幕等）、大小范围、来源链接多维度筛选；FTS5 + LIKE 双路模糊搜索，支持部分文件名匹配
