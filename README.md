...
![软件截图](HapiGo_2026-05-03_04.54.23.png)

## 功能

- **分享链接解析**：输入115分享链接，自动递归解析所有文件/目录信息；支持URL中自动提取提取码（`?password=xxx`）
- **资源库**：卡片式展示已完成的分享链接，点击进入可浏览文件夹结构、按类型筛选、搜索、转存
- **本地资源库**：文件元数据存储在本地 SQLite 数据库，不依赖网络即可搜索
- **高级筛选搜索**：按文件名、类型（视频/音频/图片/文档/ISO原盘/字幕/文件夹等）、大小范围、来源链接多维度筛选；FTS5 + LIKE 双路模糊搜索，支持部分文件名匹配
- **智能限速 + 反封控**：1-4秒随机请求间隔，WAF 检测自动指数退避（带抖动），支持代理轮换，浏览器指纹伪装（UA轮换、sec-*头、native-tls）
- **多代理支持**：HTTP / HTTPS / SOCKS5 代理，配置多个代理后可在被封时自动切换
- **账号登录**：Cookie 登录，登录后可解析带权限的分享、一键转存文件/文件夹到自己的115网盘
- **一键转存**：登录后，在账号管理中选择目标文件夹，在仪表盘/搜索/资源库中点击「转存」；支持转存整个文件夹，保留完整目录结构
- **文件夹类型识别**：解析时子文件夹标记为 `folder` 类型，支持按文件夹筛选和转存
- **分页浏览**：仪表盘和搜索页支持分页，可自定义每页条数（10/20/50/100）
- **解析日志**：实时展示解析过程，包括目录扫描、文件统计、WAF拦截警告
...
3. **添加分享链接**：在「分享链接」页面点击「添加链接」，输入115分享链接URL；如果链接中包含提取码（`?password=xxx`）会自动识别
4. **自动解析**：应用自动递归解析链接中的所有文件和目录存入本地数据库（大型分享可能较慢，可调低限速避免被封）
5. **浏览文件**：在「资源库」进入分享链接，浏览文件夹结构，按类型筛选（视频/文件夹/文档等）
6. **高级搜索**：在「搜索」页面多维度筛选（类型、大小、来源等），支持模糊匹配
7. **选择转存目录**：在「账号管理」→「转存目标目录」展开目录树，点击「选择」设定转存目的地
8. **一键转存**：在资源库/仪表盘/搜索结果中点击「转存」按钮；文件夹会保留完整目录结构转存到目标目录
...
- [x] 可配置限速 + WAF 退避 + 代理轮换
- [x] 仪表盘 + 高级搜索页面
- [x] 资源库页面（卡片式分享链接浏览 + 文件夹导航）
- [x] Cookie 登录 + 账号管理
- [x] 一键转存到指定目录（文件 + 文件夹转存，保留目录结构）
- [x] 多代理支持（HTTP/HTTPS/SOCKS5）
- [x] Mock/录制模式离线开发
- [x] FTS5 + LIKE 双路模糊搜索
- [x] 分页浏览（仪表盘/搜索，支持切换每页条数）
- [x] 反WAF：UA轮换、浏览器指纹、native-tls、备用端点
- [x] 文件类型扩展：ISO原盘、字幕文件
- [x] WAF拦截实时提醒
- [x] 文件夹类型识别（解析时标记 folder 类型，支持筛选和转存）
- [x] URL自动提取提取码（支持 ?password=xxx 参数）
- [x] 解析日志模块（实时展示解析过程）
- [x] 解析进度展示（分享链接页面进度条 + 文件/目录计数）
